...
def generate_monthly_payroll(year, month, project_id=None):
    ...
    if project_id is None:
        ...
        df["net_pay"] = (df["salary"] + df["overtime_pay"]).round(2)

        df = df.rename(
            columns={
                "days_present_all": "days_present",
                "total_hours_all": "total_hours",
                "overtime_hours_all": "overtime_hours",
            }
        )
    ...
    df = df.merge(summary_all[["worker_id", "total_hours_all"]], on="worker_id", how="left")
    df["total_hours_all"] = df["total_hours_all"].fillna(0)
    df["days_in_month"] = days_in_month
    df["hourly_rate"] = df["salary"] / (days_in_month * STANDARD_DAILY_HOURS)
    ...
    def _base_share(row):
        if row["total_hours_all"] <= 0:
            return 0.0
        return row["salary"] * (row["total_hours"] / row["total_hours_all"])

    df["base_share"] = df.apply(_base_share, axis=1)
    df["overtime_pay"] = (
        df["hourly_rate"] * df["overtime_hours"] * OVERTIME_MULTIPLIER
    ).round(2)
    df["net_pay"] = (df["base_share"] + df["overtime_pay"]).round(2)
...
def dashboard_page():
    ...
    # Project stats: assigned workers from project_workers,
    # present workers from attendance (any worker, not only assigned)
...
def main():
    ...
    # Radio has its own key; we then store result in session_state["page"]
    ...
    # Show current data path in sidebar
    st.sidebar.markdown("---")
